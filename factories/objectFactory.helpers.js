...
const _graphicsDrawers = {
  ...
  pillbox: (scene, x, y, bodyW, bodyH, cfg) => {
    ...
    const obj = new Pillbox(scene, x, y, texKey);
  ...
  mortar: (scene, x, y, bodyW, bodyH, cfg) => {
    ...
    const obj = new Mortar(scene, x, y, texKey);
    obj._factoryScaled = true;
    return obj;
  },
};

function _buildVisual(scene, cfg, x, y, bodyW, bodyH, scaleX, scaleY, type) {
...
// HP, weapon, and capability systems are now fully encapsulated inside the individual Entity classes (DestructibleEntity, Attacker, Building, Player, Plane, Pillbox, Mortar).
